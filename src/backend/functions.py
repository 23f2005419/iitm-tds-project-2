...
async def execute_commands(command: str, ) -> str:
    """
    Execute a shell command and return its output. Use this for questions about running terminal commands like 'code -s' to check VS Code status.

    Args:
        command (str): The command to execute (e.g., 'code -s', 'ls', 'dir').

    Returns:
        str: The simulated response of the command, or "Command not found" if the command is not in our predefined responses.
    """
    print('Inside the execute commands function')
    stripped_command = command.strip()
    
    if 'code -s' in stripped_command:
        return ga1_q1_vs_code_command(command)
    ...
    if "prettier" in stripped_command and "sha256sum" in stripped_command:
        ...
        if file_match:
            ...
            return await ga1_q3_calculate_prettier_sha256(filename)
        else:
...
async def ga1_q1_vs_code_command(command: str):
    """
    Simulate the response of a VS Code command by looking up the command in our predefined responses.

    Args:
        command (str): The command to simulate (e.g., 'code -s', 'ls', 'dir').

    Returns:
        str: The simulated response of the command, or "Command not found" if the command is not in our predefined responses.
    """
    print('Inside the ga1_q1_vs_code_command function')
    stripped_command = command.strip()
    if stripped_command in command_responses:
        return command_responses[stripped_command]
...
async def ga1_q2_api_request(url: str, method: str, headers: Optional[Dict[str, str]] = None, data: Optional[Dict[str, Any]] = None,) -> str:
    """
    Make an asynchronous API request to a specified URL using the specified HTTP method.

    Args:
        url (str): The URL to which the API request will be made.
        ...
        data (Optional[Dict[str, Any]]): Optional data to include in the request body for POST requests.

    Returns:
        str: The API response as a string. If the response is in JSON format, it is returned as a formatted JSON string; otherwise, the raw response text is returned.

    Raises:
        Exception: If there is an error making the API request.
    """
    print('Inside the ga1_q2_api_request function')
    try:
...
async def ga1_q3_calculate_prettier_sha256(filename: str) -> str:
    """
    ...
    """
    print('Inside the ga1_q3_calculate_prettier_sha256 function')
    try:
        import hashlib
        import subprocess
        import tempfile
...
def ga1_q4_q5_calculate_spreadsheet_formula(formula: str, type: str) -> str:
    """
    ...
    """
    print('Inside the ga1_q4_q5_calculate_spreadsheet_formula function')
    try:
...
def ga1_q7_count_days_of_week(start_date: str, end_date: str, day_of_week: str) -> str:
    """
    ...
    """
    print('Inside the ga1_q7_count_days_of_week function')
    try:
        # Parse the dates
...
async def ga1_q8_extract_zip_and_read_csv(file_path: str, column_name: Optional[str] = None) -> str:
    """
    Extract a zip file and read a value from a CSV file inside it

    Args:
        file_path (str): Path to the zip file
        column_name (Optional[str]): Column name to extract value from

    Returns:
        str: The extracted value if a column name is specified, or a summary of the CSV if no column is specified
    """
    print('Inside ga1_q8_extract_zip_and_read_csv function')
...
def ga1_q9_sort_json_array(json_array: str, sort_keys: list) -> str:
    """
    ...
    """
    print('Inside the ga1_q9_sort_json_array function')
    try:
...
async def ga1_q10_convert_keyvalue_to_json(file_path: str,question: str) -> str:
    """
...
async def ga1_q12_process_encoded_files(file_path: str, target_symbols: list) -> str:
    """
...
async def ga1_q17_compare_files(file_path: str) -> str:
    """
...
def ga1_q18_run_sql_query(query: str) -> str:
    """
...
def ga2_q1_generate_markdown_documentation(topic: str, elements: Optional[List[str]] = None) -> str:
    """
    ...
    """
    print('Inside ga2_q1_generate_markdown_documentation function')
    try:
...
async def ga2_q2_compress_image(file_path: str, target_size: int = 1500) -> str:
    """
    ...
    """
    print('Inside the ga2_q2_compress_image function')
    try:
...
async def ga2_q3_create_github_pages(email: str, content: Optional[str] = None) -> str:
    """
    ...
    """
    print('Inside the ga2_q3_create_github_pages function')
    try:
...
async def ga2_q4_run_colab_code(code: str, email: str) -> str:
    """
    ...
    """
    print('Inside the ga2_q4_run_colab_code function')
    try:
...
async def ga2_q5_analyze_image_brightness(file_path: str, threshold: float = 0.937) -> str:
    """
    ...
    """
    print('Inside the ga2_q5_analyze_image_brightness function')
    try:
...
async def ga2_q6_deploy_vercel_app(data_file: str, app_name: Optional[str] = None) -> str:
    """
    ...
    """
    print('Inside the ga2_q6_deploy_vercel_app function')
    try:
        # Generate code for a basic HTML page
        html_content = f"""<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <p>Data from {data_file}</p>
        ...
        # Generate `index.html` file and `vercel.json` file
        deployment_dir = tempfile.mkdtemp()
        with open(os.path.join(deployment_dir, "index.html"), "w") as f:
            f.write(html_content)
        with open(os.path.join(deployment_dir, "vercel.json"), "w") as f:
            json.dump(vercel_config, f, indent=4)

        # Deploy to Vercel
        import subprocess
        cmd = ["vercel", "--prod", "--token", os.getenv("VERCEL_TOKEN"), deployment_dir]
...
async def ga2_q7_create_github_action(email: str, repository: Optional[str] = None) -> str:
    """
    ...
    """
    print('Inside the ga2_q7_create_github_action function')
    try:
...
async def ga2_q8_create_docker_image(
    ...
    """
    print('Inside the ga2_q8_create_docker_image function')
    try:
...
async def ga2_q9_filter_students_by_class(file_path: str, classes: List[str] = []) -> str:
    """
    ...
    """
    print('Inside the ga2_q9_filter_students_by_class function')
    try:
        # Enable CORS
        import subprocess
        import sys
        ...
        @app.get("/api")
        async def filter_students_by_class_endpoint(request: Request):
            # Get the classes from the query parameter
            query_classes = request.query_params.getlist("class")
            if query_classes:
            ...
            with open(file_path, "r") as file:
                ...
                filtered_students = []
                for row in reader:
                    if row[header.index("Class")] in classes:
...
async def ga2_q10_setup_llamafile_with_ngrok(
    ...
    Returns:
        Setup instructions
    """
    print('Inside the ga2_q10_setup_llamafile_with_ngrok function')
    try:
...
async def ga3_q1_analyze_sentiment(text: str, api_key: str = "dummy_api_key") -> str:
    """
    ...
    Returns:
        Sentiment analysis result and API request details
    """
    print('Inside the ga3_q1_analyze_sentiment function')
...
async def ga3_q2_count_tokens(text: str) -> str:
    """
    ...
    Returns:
        str: A markdown-formatted string with the token count analysis result and API request details
    """
    
    print('Inside the ga3_q2_count_tokens function')
    url = "https://api.openai.com/v1/chat/completions"
...
async def ga3_q3_generate_structured_output(prompt: str, structure_type: str) -> str:
    """
    Generate a JSON body for OpenAI's chat API to generate structured output
    for a given prompt and structure type.

    Args:
        prompt (str): The prompt to generate structured output for
        structure_type (str): The type of structure to generate (e.g. addresses, products)

    Returns:
        str: A markdown-formatted string with the JSON body and API request details
    """
    print('Inside the generate_structured_output function')
...
"""
async def ga3_q4_generate_vision_api_request(image_url: str) -> str:
    """
    ...
    """
    print('Inside the ga3_q4_generate_vision_api_request')
    try:
        import json
...
async def ga3_q5_generate_embeddings_request(texts: List[str]) -> str:
    """
...
async def ga3_q6_find_most_similar_phrases(embeddings_dict: Dict[str, List[float]]) -> str:
    """
    ...
    """
    print('Inside the ga3_q6_find_most_similar_phrases')
    try:
...
async def ga3_q8_parse_function_call(query: str) -> str:
    """
    ...
    """
    print('Inside the ga3_q8_parse_function_call')
    try:
        import re
        import json
...
async def ga4_q1_count_cricket_ducks(page_number: int = 3) -> str:
    """
    ...
    """
    print('Inside the ga4_q1_count_cricket_ducks function')
    try:
...
async def ga4_q2_get_imdb_movies(min_rating: float = 7.0, max_rating: float = 8.0, limit: int = 25) -> str:
    """
    ...
    """
    print('Inside the ga4_q2_get_imdb_movies function')
    try:
...
async def ga4_q3_generate_country_outline(country: str) -> str:
    """
    ...
    """
    print('Inside the ga4_q3_generate_country_outline function')
    try:
...
async def ga4_q4_get_weather_forecast(city: str) -> str:
    """
    ...
    """
    print('Inside the ga4_q4_get_weather_forecast function')
    try:
...
async def ga4_q8_create_github_action_workflow(email: str, repository_url: str = None) -> str:
    """
...
async def ga4_q9_extract_tables_from_pdf(file_path: str) -> str:
    """
...
async def ga4_q10_convert_pdf_to_markdown(file_path: str) -> str:
    """
...
async def ga5_q1_clean_sales_data_and_calculate_margin(file_path: str, cutoff_date_str: str, product_filter: str, country_filter: str) -> str:
    """
    ...
    """
    print('Inside ga5_q1_clean_sales_data_and_calculate_margin')
    try:
...
async def ga5_q2_count_unique_students(file_path: str) -> str:
    """
    ...
    """
    print('Inside the ga5_q2_count_unique_students function')
    try:
...
async def ga5_q3_analyze_apache_logs(
    file_path: str,
    ...
    """
    print('Inside the ga5_q3_analyze_apache_logs function')
    try:
...
async def ga5_q4_analyze_bandwidth_by_ip(
    file_path: str,
    section_path: str = None,
    ...
    """
    print('Inside ga5_q4_analyze_bandwidth_by_ip')
    try:
        # Open the file (handling gzip if needed)
        if file_path.endswith(".gz"):
            open_func = gzip.open
...
async def ga5_q7_count_json_key_occurrences(file_path: str, target_key: str) -> str:
    """
    ...
    """
    print('Inside the ga5_q7_count_json_key_occurrences function')
    try:
        # Load the JSON file
        with open(file_path, "r", encoding="utf-8") as file:
            data = json.load(file)
...
async def ga5_q10_reconstruct_scrambled_image(image_path: str, mapping_data: str, output_path: str = None) -> str:
    """
    ...
    """
    print('Inside the ga5_q10_reconstruct_scrambled_image function')
    try:
...
def generate_duckdb_query(
    ...
    """
    Generates a DuckDB query based on the provided parameters.

    Args:
        query_type (str): The type of query to generate (e.g. "SELECT", "INSERT", etc.)
        timestamp_filter (str): A filter clause for the timestamp column (e.g. "timestamp > '2022-01-01'")
        numeric_filter (str): A filter clause for a numeric column (e.g. "value > 10")
        sort_order (str): The sort order for the query (e.g. "ASC", "DESC")

    Returns:
        str: The generated DuckDB query
    """

    # Start with the base query
    query = f"{query_type} * FROM table_name"

    # Add timestamp filter if provided
    if timestamp_filter:
        query += f" WHERE {timestamp_filter}"

    # Add numeric filter if provided
    if numeric_filter:
        if "WHERE" in query:
            query += f" AND {numeric_filter}"
        else:
            query += f" WHERE {numeric_filter}"
